...
def build_outputs() -> None:
    ...
    seed_count = int(seed_rows.loc[seed_rows["model"] == FINAL_MODEL_NAME, "seed"].dropna().nunique())
    ...
    rppo_wins = int(win_counts.get(FINAL_MODEL_NAME, 0))
    buyhold_wins = int(win_counts.get("Buy&Hold", 0))
    top_avg_model = str(overall.sort_values("avg_final_nav", ascending=False)["model"].iloc[0])
        ...
        [
            {
                ...
                "note": f"Top average model: {top_avg_model}.",
            },
            {
                "metric": "Best-stock count",
                "value": rppo_wins,
                "note": f"Transaction-Cost-Aware R-PPO ranks first by final NAV on {rppo_wins} of {len(STOCK_ORDER)} stocks.",
            },
            {
            ...
            },
            {
                ...
                "note": f"Buy&Hold ranks first on {buyhold_wins} of {len(STOCK_ORDER)} stocks.",
            },
        ]
    )
    ...
    method_notes = [
        {
            "topic": "Main model",
            "note": "Transaction-Cost-Aware R-PPO is implemented as standard SB3 PPO with a multi-frequency LSTM-based feature extractor, turnover_penalty=0.005, and rebalance_threshold=0.05.",
        },
        {
            "topic": "Seeds",
            "note": f"R-PPO, PPO (Daily), and A2C (Daily) are evaluated with {seed_count} seeds. Stock-level comparisons use seed-mean metrics.",
        },
        {
            "topic": "Benchmarks",
        ...
        },
        {
            ...
            "note": f"Transaction-Cost-Aware R-PPO wins on {rppo_wins} of {len(STOCK_ORDER)} stocks. Buy&Hold can remain best in strong upward-trending stocks, which is expected for passive exposure.",
        },
    ]
    ...
    workbook_xlsx = RESULT_DIR / f"{RESULT_PREFIX}_Thesis_Tables.xlsx"
    with pd.ExcelWriter(workbook_xlsx, engine="openpyxl") as writer:
        for name, df in outputs.items():
            df.to_excel(writer, sheet_name=name[:31], index=False)
        metadata = pd.DataFrame(
            [
                {"field": "title", "value": "Final Transaction-Cost-Aware R-PPO Thesis Tables"},
                {"field": "source_seed_rows", "value": str(SEED_ROWS_CSV)},
                {"field": "source_summary", "value": str(SUMMARY_CSV)},
                {"field": "stock_count", "value": len(STOCK_ORDER)},
                {"field": "seed_count", "value": seed_count},
            ]
        )
        metadata.to_excel(writer, sheet_name="metadata", index=False)

    md = build_markdown_summary(overall, win_counts_df, stat_report, stock_rankings, seed_count)
...
def build_markdown_summary(
    ...
    rppo_wins = int(win_counts_df.loc[win_counts_df["model"] == FINAL_MODEL_NAME, "best_stock_count"].iloc[0])
    buyhold_wins = int(win_counts_df.loc[win_counts_df["model"] == "Buy&Hold", "best_stock_count"].iloc[0])

    return f"""# Final Transaction-Cost-Aware R-PPO Thesis Summary

## Experimental setup

- Main model: Transaction-Cost-Aware R-PPO, implemented as standard SB3 PPO with a multi-frequency LSTM-based feature extractor.
...
- Seeds: R-PPO, PPO (Daily), and A2C (Daily) use {seed_count} random seeds.
- Benchmarks: PPO (Daily), A2C (Daily), GARCH, Buy&Hold, and Baseline (1.0) in plots.

## Main result

Across the 8-stock out-of-sample universe, Transaction-Cost-Aware R-PPO ranked first on {rppo_wins} of 8 stocks. Buy&Hold ranked first on {buyhold_wins} stocks, which is expected when passive full exposure benefits from strong upward trends.
...
Across a balanced eight-stock test universe and {seed_count} random seeds, Transaction-Cost-Aware R-PPO provides a transaction-cost-aware PPO strategy with multi-frequency LSTM feature extraction. The results should be interpreted as evidence of improved average robustness over PPO, A2C, and GARCH baselines under the tested setting. However, Buy&Hold remains difficult to beat during strong upward trends, so the thesis should claim overall and condition-dependent improvement rather than universal dominance.
