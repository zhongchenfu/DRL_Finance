...
class MultiFreqStockEnv(gym.Env):
    ...
    def __init__(
        ...
        if drawdown_penalty_coef < 0.0:
            raise ValueError("drawdown_penalty_coef must be non-negative.")
        ...
        self.use_high_frequency = bool(use_high_frequency)
        ...
        high_state_dim = self.high_len * self.high_features if self.use_high_frequency else 0
        self.state_dim = (
        ...
        self.peak_asset_value = self.initial_amount
        self.reward = 0.0
    ...
    def _validate_lengths(self) -> None:
        ...
        if len(self.df_low) == 0:
            raise ValueError("df_low must not be empty.")
        if self.use_high_frequency and len(self.df_high) == 0:
            raise ValueError("df_high must not be empty when high-frequency observations are enabled.")
    ...
    def _get_state(self) -> np.ndarray:
        ...
        if self.use_high_frequency:
            ...
            state_parts.append(np.nan_to_num(high_state).flatten())
        state_parts.append(portfolio_state)
    ...
    def step(self, action):
        ...
        self.peak_asset_value = max(self.peak_asset_value, new_asset)
        current_drawdown = (
            max(self.peak_asset_value - new_asset, 0.0) / self.peak_asset_value
            if self.peak_asset_value > 0
            else 0.0
        )
        drawdown_penalty = self.drawdown_penalty_coef * current_drawdown
        self.reward = float(np.clip(raw_reward - turnover_penalty - drawdown_penalty, -REWARD_CLIP, REWARD_CLIP))
