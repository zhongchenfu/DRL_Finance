...
# Train/test split used by all experiment variants.
TRAIN_START = "2020-03-01"
TRAIN_END = "2023-12-31"
TEST_START = "2024-01-01"
TEST_END = "2024-12-31"

STOCKS = {"sh.601628": "China Life", "sh.601688": "Huatai Securities", "sh.600519": "Kweichow Moutai"}
# ================================
# Configuration
# ================================
CONFIG = {
    "TRAIN_MODELS": False,  # True: train and save models; False: load saved models for evaluation.
    "MODELS_TO_RUN": ["R-PPO", "PPO", "A2C", "GARCH"],
    # Example: "MODELS_TO_RUN": ["A2C"]
}
...
def train_baseline_a2c(env_train):
    ...
    model = A2C("MlpPolicy", env_train, 
                n_steps=100,  # Use a longer rollout horizon than the A2C default.
                learning_rate=0.0003, 
                verbose=0)
    model.learn(total_timesteps=550000, progress_bar=True)
    return model
...
def main():
    ...
    for code, name in STOCKS.items():
        ...
        # Use a consistent color map and make R-PPO visually prominent.
        model_colors = {
            'R-PPO': '#D62728',
            'PPO': '#1F77B4',
            'A2C': '#2CA02C',
        ...
        # Plot the normalized 1.0 reference line.
        ...
        plt.title(f'Cumulative Return Comparison: {name} ({code})', fontsize=18, fontweight='bold', pad=20)
        plt.xlabel('Date', fontsize=14)
        plt.ylabel('Cumulative Return (Normalized)', fontsize=14)
        
        # Place the legend outside the plot area to keep the curves readable.
        plt.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=True, fontsize=12)
        
        # Keep the chart clean for reporting.
        plt.grid(True, linestyle='--', alpha=0.6)
        sns.despine(left=True, bottom=True)
        
        # Format date labels consistently across generated charts.
