...
plt.rcParams['font.sans-serif'] = ['WenQuanYi Micro Hei']  # Use the installed Chinese font.
...
# STOCKS = {"sh.600519": "Kweichow Moutai"}
# ================================
# Configuration
# ================================
CONFIG = {
    "TRAIN_MODELS": True,  # True: train new models and save them; False: load existing weights for evaluation
    "MODELS_TO_RUN": ["R-PPO", "PPO", "A2C", "GARCH"], # Configure which models to run
    # "MODELS_TO_RUN": [ "A2C"], # Configure which models to run
...
def main():
    ...
    for initial_amount, (code, name) in (
        ...
        # Define the color mapping and make R-PPO stand out.
        model_colors = {
            'R-PPO': '#D62728',   # red (highlight)
            'PPO': '#1F77B4',     # blue
            'A2C': '#2CA02C',     # green
            'GARCH': '#9467BD',   # purple
            'Buy & Hold': '#7F7F7F' # gray (benchmark)
        }

        # Plot the 1.0 baseline.
        ...
        plt.savefig(img_path, dpi=300, bbox_inches='tight') # Increase resolution.
        plt.close()
