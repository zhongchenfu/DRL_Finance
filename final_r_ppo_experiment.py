...
CONFIG = {
    ...
    "R_PPO_SEEDS": list(range(15)),
    "BASELINE_SEEDS": list(range(15)),
...
def backtest_garch(
    ...
    for i in range(start_idx, len(df_test) - 1):
        ...
        if len(returns_history) >= max(2, window_size):
            ...
            pred_next_var = float(forecasts.variance['h.1'].iloc[-1])
            pred_next_vol = float(np.sqrt(max(pred_next_var, 0.0)))

        if pred_next_ret > 0:
            if np.isfinite(pred_next_vol) and pred_next_vol > 1e-8:
                vol_scaled_weight = target_daily_vol_pct / pred_next_vol
                target_weight = float(np.clip(vol_scaled_weight, min_positive_weight, max_position_weight))
            else:
                target_weight = float(max_position_weight)
        else:
            target_weight = 0.0

        asset_value = cash + shares * current_price
        target_shares = int((target_weight * asset_value) / current_price)
        target_shares = (target_shares // 100) * 100
        ...
        if target_shares != shares:
            ...
            if diff > 0:
                ...
                buy_shares = min(diff, max(0, max_affordable))
                if buy_shares > 0:
                    cash -= buy_shares * current_price * (1 + fee)
                    shares += buy_shares
            else:
        ...
        if cash < -1e-6:
            raise RuntimeError(f"GARCH simulator produced negative cash: {cash:.6f}")
...
def _t_critical_95(seed_count: int) -> float:
    """Two-sided 95% t critical value for a seed-level mean estimate."""
    if seed_count <= 1:
        return 0.0
    critical_by_df = {
        1: 12.706,
    ...
    return critical_by_df.get(seed_count - 1, 1.96)
...
def _mean_seed_account(seed_accounts):
    ...
    merged["nav_seed_count"] = seed_count
    merged["nav_std"] = merged[seed_cols].std(axis=1, ddof=1).fillna(0.0)
    merged["nav_sem"] = merged["nav_std"] / np.sqrt(seed_count) if seed_count > 0 else 0.0
    merged["nav_ci_margin"] = _t_critical_95(seed_count) * merged["nav_sem"]
    merged["nav_lower"] = merged["nav"] - merged["nav_ci_margin"]
    merged["nav_upper"] = merged["nav"] + merged["nav_ci_margin"]
...
def final_t_critical_95(seed_count: int) -> float:
    """Two-sided 95% t critical value for the mean across seed trajectories."""
    return _t_critical_95(seed_count)
...
def final_mean_seed_account(accounts_by_seed: dict[int, pd.DataFrame]) -> pd.DataFrame:
    ...
    merged["nav_std"] = merged[seed_cols].std(axis=1, ddof=1).fillna(0.0)
    merged["nav_sem"] = merged["nav_std"] / np.sqrt(seed_count) if seed_count > 0 else 0.0
    merged["nav_ci_margin"] = final_t_critical_95(seed_count) * merged["nav_sem"]
    merged["nav_lower"] = merged["nav"] - merged["nav_ci_margin"]
    merged["nav_upper"] = merged["nav"] + merged["nav_ci_margin"]
...
def final_plot_comparison(
    code: str,
    stock: str,
    account_values: dict[str, pd.DataFrame],
    mean_accounts: dict[str, pd.DataFrame],
    seed_account_values: dict[str, dict[int, pd.DataFrame]] | None = None,
) -> Path:
    ...
    plt.figure(figsize=(12, 7))
    plt.axhline(y=1.0, color="#6E6E6E", linestyle="--", linewidth=1.5, alpha=0.9, label="Baseline (1.0)")
    seed_account_values = seed_account_values or {}
    ...
    for model_name in [FINAL_MODEL_NAME, "PPO (Daily)", "A2C (Daily)"]:
        for _, seed_account in sorted(seed_account_values.get(model_name, {}).items()):
            if seed_account is None or seed_account.empty:
                continue
            seed_nav = _account_with_nav(seed_account)
            plt.plot(
                seed_nav["date"],
                seed_nav["nav"],
...
def final_run_stock(code: str, stock: str) -> list[dict]:
    ...
    account_values: dict[str, pd.DataFrame] = {}
    mean_accounts: dict[str, pd.DataFrame] = {}
    seed_account_values: dict[str, dict[int, pd.DataFrame]] = {}
    ...
    seed_account_values[FINAL_MODEL_NAME] = rppo_accounts
    ...
    for model_key, label in [("ppo_daily", "PPO (Daily)"), ("a2c_daily", "A2C (Daily)")]:
        ...
        seed_account_values[label] = baseline_accounts
    ...
    plot_path = final_plot_comparison(code, stock, account_values, mean_accounts, seed_account_values)
